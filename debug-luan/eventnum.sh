    ...
    # for SEED in 2021; do
      for bfsz in 200 ; do
        for rwd in 3; do
          ...
          PBUF_SIZE=${bfsz} REWARDF=${rwd} TEST=${test_name}-s${SEED} ${TOOL} --disable-mm-detector  -estimation-min=${ESTIMATE} -estimation-max=${ESTIMATE} ${tool_options} -print-estimation-stats --schedule-seed=${SEED} -schedule-policy=arbitrary -estimation-threshold=0 -- ${extra_options} ${source_file} 2>&1 # | grep -E "Estimatioin time elapsed"
        done
      done
