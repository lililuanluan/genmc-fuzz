...
#ifdef DEBUG_LUAN
	using Thread = std::pmr::vector<std::unique_ptr<EventLabel> >;
	using ThreadList = std::pmr::vector<Thread>;
	using StoreList = llvm::simple_ilist<WriteLabel>;
	using LocMap = std::pmr::unordered_map<SAddr, StoreList>;
#else 
...
#ifdef DEBUG_LUAN
public:	// use the setEventLabel function
	void eraseEvent(Event e) {
		auto& t = events[e.thread];
		auto it = t.begin() + e.index;
		t.erase(it);
	}
#endif
