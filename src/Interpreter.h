...
#include "GenMCDriver.hpp"  // luan: use driver pointer
...
#define CALL_DRIVER_RESET_IF_FALSE(method, ...)			\
	({							\
        ...
        Event incPos() {
// #ifdef DEBUG_LUAN
//             Print("incPos", GREEN("currPos() = "), currPos(), "graph: ", driver->getGraph());
//             // getchar();
// #endif
            auto& thr = getCurThr();
// #ifdef DEBUG_LUAN
//             Print("after getCurThr, thr.globalInstructions = ", thr.globalInstructions);

// #endif
            auto ins = ++thr.globalInstructions;
// #ifdef DEBUG_LUAN
//             Print("ins = ++thr.globalInstructions = ", ins);

// #endif
            return Event(thr.id, ins);
