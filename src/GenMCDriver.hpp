...
public:
	void explore_dbg();
	using RFPair = std::pair<const ReadLabel*, Event>;
	...
	// minimal:
	void minimalCut();
	void do_minimalCut(RFPair rf, const ExecutionGraph& g, bool push_to_gc = false);
	void minimalCut_n(bool push_to_gc = false);

	// maximal:
	void maximalCut();
	void maximalCut_n() {}
	void do_maximalCut(RFPair rf, const ExecutionGraph& g);
	template<bool PushOnce = true>
	void maximalCut2();	// looser than maximalcut
	template<typename T, typename U>
	void do_maximalCut2(RFPair rf, const ExecutionGraph& g, T&&, U&&);
	void maximalCut2_n();

	void maximalCutRMW();	// seems tobe a broken one
	template<bool PushOnce = true>
	void maximalCutRMW2();
	void do_maximalCutRMW2(RFPair rf, const ExecutionGraph& g);



	// mixed: just for counting events
	void mixCutDemo();


	inline static const int push_num = []()
		{
			auto p = std::getenv("PUSHNUM");
			return p ? std::atoi(p) : 1;
		}();

	inline static const int mut = []()
		{
	...
	inline static const bool halt_on_error = []()
		{
			auto s = std::getenv("HALT_ON_ERROR");
			return (bool)s;
		}();

	inline static const size_t time_out = []()
		{
			auto s = std::getenv("TIME_OUT_SEC");
			return s ? std::stoul(s) : std::numeric_limits<size_t>::max();
		}();

	void powerSched();

	static Stamp getMaxStamp(const ExecutionGraph& g) {
		Stamp maxStamp = { 0 };
		for (auto i = 0u; i < g.getNumThreads(); i++) {
			for (auto j = 0u; j < g.getThreadSize(i); j++) {
				Event e(i, j);
				auto s = g.getEventLabel(e)->getStamp();
				maxStamp = std::max(s, maxStamp);
			}
		}
		return maxStamp;
	}
	...
	void pushGraph(std::unique_ptr<ExecutionGraph>& og) {
		auto m = createChoiceMapForCopy(*og);
		auto maxStamp = getMaxStamp(*og);
		og->resetStamp(maxStamp + 1);
		Print("maxStamp:", maxStamp);
		Print("og(to be pushed):", *og);
		// getchar();
		pushExecution({ std::move(og), LocalQueueT(), std::move(m) });
		addToWorklist(maxStamp, std::make_unique<RerunForwardRevisit>());
	}

	void printPrefixViewsOnStack(std::string_view msg = "") {
		Print(msg);
		Print("prefixViews:");
		auto& g = getGraph();
		for (auto i = 0u; i < g.getNumThreads(); i++) {
	...
	std::optional<RFPair> selectAlternativeRF(const ExecutionGraph& g, const M& pick, const R& rmv) {


		if (auto p = std::getenv("PRIORITIZE_RF")) {
			auto pri = std::atoi(p);
			// prioritize larger stamps
			if (pri == 1) {
				...
				for (auto i = 0u; i < g.getNumThreads(); i++) {
					for (auto j = 0u; j < g.getThreadSize(i); j++) {
						...
						const EventLabel* lab = g.getEventLabel(e);
						BUG_ON(!lab);
						if (auto rLab = pick(lab, g)) {
							BUG_ON(!rLab);
							// Print("getting rf approx for:", *rLab);
							auto stores = getRfsApproximation(rLab);
							// Print("erasing some stores");
							stores.erase(std::remove_if(stores.begin(), stores.end(), rmv(rLab, g)), stores.end());
							if (stores.size() > 0) {
								for (auto& s : stores) {
				...
				std::optional<RFPair> res;
				if (rf_options.size() > 0) {
					size_t totalWeight = 0;
					std::vector<size_t> weights;
					weights.reserve(rf_options.size());

					for (const auto& rf : rf_options) {
						auto label = g.getEventLabel(rf.second);
						BUG_ON(!label); // Ensure label is valid
						size_t stamp = label->getStamp().get();
						weights.push_back(stamp);
						totalWeight += stamp;
					}

					std::discrete_distribution<> dist(weights.begin(), weights.end());
					auto idx = dist(estRng);  // Generate random index based on weights
					res = rf_options[idx];    // Select the RFPair corresponding to the index
				}
				return res;
			}
			...
			// prioritize rmw
				...
				std::vector<RFPair> rf_options;
				std::vector<RFPair> rf_options_rmw;
				for (auto i = 0u; i < g.getNumThreads(); i++) {
					for (auto j = 0u; j < g.getThreadSize(i); j++) {
						const Event e(i, j);
						const EventLabel* lab = g.getEventLabel(e);
						...
						if (auto rLab = pick(lab, g)) {
							...
							if (stores.size() > 0) {
								for (auto& s : stores) {
									if (g.isRMWLoad(rLab)) {
										rf_options_rmw.push_back({ rLab, s });
									}
									else {
										rf_options.push_back({ rLab, s });
									}
				...
				std::optional<RFPair> res;
				if (rf_options_rmw.size()) {
					llvm::outs() << "pri rmw\n";
					MyDist dist(0, rf_options_rmw.size() - 1);
					res = rf_options_rmw[dist(estRng)];
					return res;
				}
				else if (rf_options.size() > 0) {
					MyDist dist(0, rf_options.size() - 1);
					res = rf_options[dist(estRng)];
				}
				return res;
	...
	template<typename M, typename R>
	std::vector<RFPair> selectAlternativeRF_n(const ExecutionGraph& g, const M& pick, const R& rmv, size_t num_select) {
		std::vector<RFPair> rf_options;
		for (auto i = 0u; i < g.getNumThreads(); i++) {
			for (auto j = 0u; j < g.getThreadSize(i); j++) {
				const Event e(i, j);
		...
		std::vector<RFPair> selected_rf;
		if (!rf_options.empty() && num_select > 0) {
			std::random_device rd;
			std::mt19937 gen(rd());
			if (num_select >= rf_options.size()) {
				selected_rf = rf_options;
			}
			else {
				std::sample(rf_options.begin(), rf_options.end(), std::back_inserter(selected_rf),
					num_select, gen);
			}
		}
	...
	GraphCounter& getGraphCounter() const {
		BUG_ON(!gc);
		return *gc;
	}


	GraphCounter* gc = nullptr;
#ifdef REORDER_RMW
	WriteLabel* completeRevisitedRMW(const ReadLabel* rLab, ExecutionGraph& g);
	EventLabel* addLabelToGraph(std::unique_ptr<EventLabel> lab, ExecutionGraph& g);
#endif	// REORDER_RMW
#endif
...
protected:
#ifdef DEBUG_LUAN
	using LocalQueueT = std::pmr::map<Stamp, WorkSet>;
	using ValuePrefixT = std::pmr::unordered_map<unsigned int,
		Trie<std::vector<SVal>,
		std::vector<std::unique_ptr<EventLabel>>,
		SValUCmp>
	>;
	using ChoiceMap = std::pmr::map<Stamp, VSet<Event>>;
#else
...
public:
	...
	bool shouldStopEstimating() {
		// #ifdef DEBUG_LUAN
		// 		// Print("checking should stop estimation");
		// #endif
...
#ifdef DEBUG_LUAN
		auto should = (totalExplored >= getConf()->estimationMax);
		// Print("should stop est:", should ? "yes" : "no", "explored =", totalExplored);
		return should;
#endif
...
private:
	...
	struct Execution {
		...
		const ExecutionGraph& getGraph() const {
			BUG_ON(!graph);
			return *graph;
		}
	...
	bool isExecutionValid(const EventLabel* lab) {
		Print("isExecution Valid:", lab->getPos());
		// Print(getGraph());
	...
	std::pmr::deque<Execution> execStack{ &mem_resource };	// allowing pop front...
	// std::deque<Execution> execStack;
#endif
