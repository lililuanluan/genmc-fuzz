...
{
#ifdef FUZZ_LUAN
	// static auto s = std::getenv("MUTATION");
	// static auto mut = std::atoi(s);
	// if (mut == 3) return 0;
	// Print("isDepTracking, returns 1");
	// getchar();
	return 1;
#endif
	return 0;
...
RC11Driver::getCoherentStores(SAddr addr, Event read)
{
	 ...
	 */
	auto endIt = (isDepTracking()) ? splitLocMOAfterHb(addr, read) : g.co_end(addr);	// isDepTracking() ==> true
