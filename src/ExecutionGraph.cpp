...
EventLabel* ExecutionGraph::addLabelToGraph(std::unique_ptr<EventLabel> lab)
{
	// #ifdef FUZZ_LUAN
		// ...
	BUG_ON(!lab);
	Print("addLabelToGraph: ", *lab, "has stamp? ", lab->hasStamp() ? "true" : "false", "next stamp is: ", timestamp);
	// #endif
	...
	// #ifdef FUZZ_LUAN
	// 	// ...
	Print("addLabelToGraph: ", "pos = ", pos);
	// #endif
	if (pos.index < events[pos.thread].size()) {
		auto eLab = getEventLabel(pos);
		Print("eLab = ", *eLab, ". isa empty label: ", llvm::isa<EmptyLabel>(eLab) ? "true" : "false", "events[pos.thread].size()=", events[pos.thread].size());
		BUG_ON(eLab && !llvm::isa<EmptyLabel>(eLab));
		events[pos.thread][pos.index] = std::move(lab);
	}
	else {
		// Print("pos.index", pos.index, ">= events[pos.thread].size()", events[pos.thread].size());
		// using r = decltype(events[pos.thread]);
		// Print("pos.thread =", pos.thread);
		// Print("events[pos.thread]");
		// for (auto& p : events[pos.thread]) {
		// 	if (p)
		// 		Print("    ", *p);
		// 	else Print("    ", "null");
		// }
	...
	// Print("done, getEventLabel(pos)");
	// Print(*getEventLabel(pos));
...
{
	// Print("checking", store, "isStoreReadBySettledRMW in", prefix);
	// Print("labels:");
	// for (const auto& lab : labels(*this)) {
	// 	Print("\t", lab.getPos());
	// }
	for (const auto& lab : labels(*this)) {
		...
		// 
		if (!rLab->getRf()) {
			Print("rLab:", *rLab, "is bottom:", rLab->getPos().isBottom());
			BUG_ON(!rLab->getRf());	// luan
		}
...
ExecutionGraph::getViewFromStamp(Stamp stamp) const
{
	...
	// Print("numthreads:", getNumThreads());
	for (auto i = 0u; i < getNumThreads(); i++) {
		// Print("thread ", i, "size:", getThreadSize(i));
		for (auto j = (int)getThreadSize(i) - 1; j >= 0; j--) {
			auto* lab = getEventLabel(Event(i, j));
			BUG_ON(!lab);	// luan
			// Print("getViewFromStamp: lab =", *lab);

#ifdef DEBUG_LUAN
			// luan: the label may be empty, skip it
			// if (llvm::isa<EmptyLabel>(lab)) { continue; }
#endif
...
{
	Print("cutToStamp ", stamp);
	auto preds = getViewFromStamp(stamp);
#ifdef FUZZ_LUAN
	Print("cutToStamp ", stamp, ", preds = ", *preds);
#endif
...
void ExecutionGraph::copyGraphUpTo(ExecutionGraph& other, const VectorClock& v) const
{
	...
	for (auto i = 0u; i < getNumThreads(); i++) {
		...
		if (i != 0) {
			Print("skipping the initializer, add Event(i, 0)", Event(i, 0));
			other.addLabelToGraph(getEventLabel(Event(i, 0))->clone());
		}

		Print("adding E(", i, " 1...", v.getMax(i), ")");
		for (auto j = 1; j <= v.getMax(i); j++) {
			Print("iterating:", Event(i, j));
			if (!v.contains(Event(i, j))) {
				Print("v NOT contains E(i,j), add hole and continue");
				other.addLabelToGraph(createHoleLabel(Event(i, j)));
				continue;
			}
			Print("adding", Event(i, j), "getEventLabel(Event(i, j)):", getEventLabel(Event(i, j))->getPos());
			BUG_ON(!getEventLabel(Event(i, j)));	// luan
...
#ifdef DEBUG_LUAN
	// format: stamp @ label
	// where label = (i-th thrd, j-th event in thrd i): label-kind
#endif
...
#ifdef DEBUG_LUAN
	// 
	// for each location, loc: writes
	// loc: I = internal, U = not iternal. (storage class) G = static, S = auto, H = heap. #addr
	// writes(co, mo): (i-th thrd, j-th event in thrd i)...
	if (g.loc_begin() == g.loc_end()) {
		s << "coherence empty\n";
	}
	else {
		s << "printing LocMap coherence...\n";
	}
...
#ifdef DEBUG_LUAN
void PrintCoherence(const ExecutionGraph& g) {
	static const int p = std::atoi(std::getenv("Print"));
	if (p != 1) return;
...
void PrintCoherence(const ExecutionGraph& g, SAddr addr) {
	static const int p = std::atoi(std::getenv("Print"));
	if (p != 1) return;
