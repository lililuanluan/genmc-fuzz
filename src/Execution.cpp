...
#ifdef FUZZ_LUAN
void printAPInt(const llvm::APInt& IntVal) {
	llvm::SmallVector<char, 40> Str;
	IntVal.toString(Str, 10, true); // 10进制，带符号
	std::string StrVal(Str.begin(), Str.end());
	Print(StrVal);
}
void printGenericValue(const llvm::GenericValue& GV, llvm::Type* Ty) {
	switch (Ty->getTypeID()) {
	case llvm::Type::IntegerTyID:
		Print("Integer:");
		printAPInt(GV.IntVal);
		break;
	case llvm::Type::FloatTyID:
		Print("Float:", GV.FloatVal);
		break;
	case llvm::Type::DoubleTyID:
		Print("Double:", GV.DoubleVal);
		break;
	case llvm::Type::PointerTyID:
		Print("Pointer:", GV.PointerVal);
		break;
	default:
		Print("Unknown type");
	}
...
void Interpreter::visitLoadInst(LoadInst& I)
{
	...
	// #ifdef FUZZ_LUAN
	// 	Print("visitLoadInst, thr = ", thr.id);
	// #endif
	GenericValue src = getOperandValue(I.getPointerOperand(), ECStack().back());
#ifdef FUZZ_LUAN
	// Print("visitLoadInst for ", currPos(), " src.type = ", src.);
	// Print("visitLoadInst for", currPos(), "src type =", I.getPointerOperand()->getType()->getTypeID(), "src value =");
	// printGenericValue(src, I.getPointerOperand()->getType());
...
#ifdef FUZZ_LUAN
		Print("IS a tls access.");
#endif
		SetValue(&I, thr.tls[ptr], ECStack().back());
		return;
	}
#ifdef FUZZ_LUAN
	Print("NOT a tls access.");
#endif
...
#ifdef FUZZ_LUAN
	// Print("deps ", deps == nullptr ? "nullptr" : "not null");
	// if (deps) {
	// 	Print("deps:", *deps);
	// }
	// else {
...
#ifdef FUZZ_LUAN
#define DBG_IMPL(...) Print(__VA_ARGS__)
#else
#define DBG_IMPL(...)
#endif
...
// #ifdef FUZZ_LUAN
// 	Print("after IMPLEMENT_READ_VISIT, getReadKind(I) = ", getReadKind(I), ", currPos() = ", currPos(), " ");
// #endif
...
#ifdef FUZZ_LUAN
	Print("after visiting I, val has value? ", val.has_value() ? GREEN("true") : RED("false"));
#endif
...
void Interpreter::run()
{
	...
	while (driver->scheduleNext()) {
#ifdef DEBUG_LUAN
		// Print("interpreter::run(),  while (driver->scheduleNext())...", "currPos() =", currPos());
		// getchar();
#endif
		if (driver->tryOptimizeScheduling(currPos())) {
			// Print(GREEN("tryOpt = true"));
			continue;
		}
		else {
			// Print(RED("tryOpt = false"));
		}
		...
		// Print("after visit I:", driver->getGraph());
	}
	return;
}
